...
class RequestProcessor
{
    ...
    public static function generate(Options $options, bool $nonInteractive = false): array
    {
        ...
        // $options->useEccDefaults = $options->useEccDefaults ?? true;
        // $options->eccCurveName = $options->eccCurveName ?? 'prime256v1';
        
  

    if (isset($_SERVER['argv']) && in_array('--useEccDefaults', $_SERVER['argv'])) {
        $options->useEccDefaults = true;
    }

        // Generate Private Key
        $pkeyConfig = $options->useEccDefaults
        ? [
            'private_key_type' => OPENSSL_KEYTYPE_EC,
            'curve_name' => $options->eccCurveName ?? 'prime256v1', // Default curve
        ]
        : [
            'private_key_bits' => $options->rsaKeySize ?? 2048, // Default RSA bits
            'private_key_type' => OPENSSL_KEYTYPE_RSA,
        ];
   
        $pkey = openssl_pkey_new($pkeyConfig);
        if (!$pkey) {
            throw new OpenSSLGenerationException('Failed to generate private key: ' . openssl_error_string());
        }
        ...
        if (!empty($options->targetPath)) {
            openssl_pkey_export_to_file(
                $pkey,
                $options->targetPath . DIRECTORY_SEPARATOR . "private" . $options->suffix . ".key",
        ...
        if (!empty($options->targetPath)) {
            openssl_csr_export_to_file(
                $csr,
                $options->targetPath . DIRECTORY_SEPARATOR . "csr" . $options->suffix . ".pem",
                $options->privateKeyPassword
            );
        }
        ...
        if (!$options->csrOnly) {
            if ($options->apiKey) {
                // ZeroSSL signing with validation
                ...
                if (empty($draft['id'])) {
                    echo "\nThe CA does not accept your certificate request. Check the input and try again.\n";
                ...
                if (!$options->noOut) {
                    ...
                    echo "\nFirst step successfully initiated. Now let's verify your ownership and sign it 🙂\n";
                }
    
                if ($options->targetPath) {
                    $validationPath = $options->targetPath . DIRECTORY_SEPARATOR . self::VALIDATION_SUBFOLDER;
                    if (
                        $options->validationType !== CertificateValidationType::EMAIL &&
                        !is_dir($validationPath) &&
                        !mkdir($validationPath) &&
                        !is_dir($validationPath)
                    ) {
                        throw new ConfigurationException(sprintf(
                            'Creation of validation files folder "%s" was not possible. Permission problem?',
                            $validationPath
                        ));
                    }
                }
    
                // Validation Method Handling
                switch ($options->validationType) {
                    ...
                    case CertificateValidationType::HTTPS_CSR_HASH:
                        $remoteFileInfo = "";
                        foreach ($draft["validation"]["other_methods"] as $domain => $info) {
                            $fileKey = $options->validationType === CertificateValidationType::HTTP_CSR_HASH ? "file_validation_url_http" : "file_validation_url_https";
                            if (!array_key_exists($fileKey, $info)) {
                                throw new ConfigurationException("File validation is not possible for this certificate.");
                            }
                            $currentInfo = $info[$fileKey] . "\n\n" . implode("\n", $info["file_validation_content"]) . "\n\n";
                            $tmp = explode("/", $info[$fileKey]);
                            $fileName = end($tmp);
                            file_put_contents(
                                $validationPath . DIRECTORY_SEPARATOR . $domain . "-validate-" . $options->suffix . "-" . $fileName . ".txt",
                                $currentInfo
                            );
                ...
                if ($nonInteractive && !$options->createOnly) {
                    ...
                    if (trim($line) === 'VERIFY') {
                        ...
                        echo "\nCertificate will not be signed for now. Continue with verification in the CLI separately or continue in your ZeroSSL dashboard: https://app.zerossl.com/certificate/verify/" . $hash;
                        fclose($handle);
                    }
                }
            } else {
                // Self-signing
                $certificate = SelfSigner::sign(
                    $csr,
                    $pkey,
                ...
                if (!empty($options->targetPath)) {
                    openssl_x509_export_to_file(
                        $certificate,
                        $options->targetPath . DIRECTORY_SEPARATOR . "certificate" . $options->suffix . ".pem",
                        $options->privateKeyPassword
                    );
                    openssl_pkcs12_export_to_file(
                        $certificate,
                        $options->targetPath . DIRECTORY_SEPARATOR . "certificate" . $options->suffix . ".pfx",
                        $pkey,
                        $options->privateKeyPassword
                    );
                }
